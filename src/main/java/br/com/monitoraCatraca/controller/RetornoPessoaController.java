...
public class RetornoPessoaController implements Serializable {
    ...
    public void liberar_pessoa() throws IOException {
        ...
        try {
            if (request.getParameter("cliente") != null && request.getParameter("pessoa") != null && request.getParameter("catraca") != null && request.getParameter("servidor") != null) {
                ...
                Integer nr_servidor = Integer.parseInt(request.getParameter("servidor"));

                new DaoObject(cliente).queryExecute(
                        "  DELETE FROM soc_catraca_liberada WHERE id IN \n "
                        + "(SELECT cl.id \n "
                        + "  FROM soc_catraca_liberada cl \n "
                        + " INNER JOIN soc_catraca c ON c.id = cl.id_catraca \n "
                        + " WHERE c.nr_numero = " + nr_catraca + " \n "
                        + "   AND c.nr_servidor = " + nr_servidor + " \n "
                        + ");"
                );

                if (!new DaoObject(cliente).queryExecute(
                        "INSERT INTO soc_catraca_liberada (nr_pessoa, id_catraca) \n "
                        + "(SELECT " + nr_pessoa + " AS nr_pessoa, c.id AS id_catraca \n "
                        + "  FROM soc_catraca c \n "
                        + " WHERE c.nr_numero = " + nr_catraca + " \n "
                        + "   AND c.nr_servidor = " + nr_servidor + " \n "
                        + "); \n "
        ...
        WSRetorno ws = new WSRetorno(false, "Parâmetros inválidos");
        externalContext.getResponseOutputWriter().write(new JSONObject(ws).toString());
        facesContext.responseComplete();
    }
    ...
    public void liberar_cartao() throws IOException {
        ...
        try {
            if (request.getParameter("cliente") != null && request.getParameter("cartao") != null && request.getParameter("catraca") != null && request.getParameter("servidor") != null) {
                String cliente = request.getParameter("cliente");
                ...
                if (!new DaoObject(cliente).queryExecute(
                        "INSERT INTO soc_catraca_liberada (nr_pessoa, id_catraca) \n "
                        + "(SELECT '" + st_cartao + "' AS st_cartao, c.id AS id_catraca \n "
                        + "  FROM soc_catraca c \n "
                        + " WHERE c.nr_numero = " + nr_catraca + " \n "
                        + "   AND c.nr_servidor = " + nr_servidor + " \n "
                        + "); \n "
